...
(function (parent, factory) {
    ...
    _addFileToFolderAsync: function (filename, path, arrayBuffer) {
        ...
        var url =
            sspjs.sp.$1.getApiUrl() + "web/getfolderbyserverrelativeurl('" + path + "')/files" +
            "/add(overwrite=true, url='" + fileName + "')";

        sspjs.sp.$1.ajax(url, dfd.resolve, dfd.reject, 'upload', arrayBuffer);
        ...
        return (function (scope, context) {
            scope = {
                ...
                uploadAsync: function ($sourceInput, newName, path) {
                    ...
                    var getBufferAsync = sspjs.sp._getFileBufferAsync($sourceInput[0].files[0]);
                    getBufferAsync.done(function (arrayBuffer) {
                        var addFile = sspjs.sp._addFileToFolderAsync($sourceInput[0].value, folder, arrayBuffer);
                        addFile.done(function (item, status, xhr) {
                            var uri = item.d.ListItemAllFields.__deferred.uri + '';
                            var getItemAsync = sspjs.sp.item().getByUrlAsync(uri);
